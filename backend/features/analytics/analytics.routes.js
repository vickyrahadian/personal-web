...
function createAnalyticsRouter({ dataDir, contactsPath, analyticsSalt, isAdmin }) {
  ...
  const readVisitors = () => read(visitorsPath, values => ({ visitor_id: values[0], first_seen: values[1], last_seen: values[2], ip_hash: values[3], country: values[4], city: values[5], device: values[6], browser: values[7], os: values[8], language: values[9], timezone: values[10], screen_size: values[11] }));
  ...
  router.get('/overview', (req, res) => {
    ...
    const traffic = read(trafficPath, ([timestamp, visitor_id, session_id, event, routePath, referrer, duration_ms]) => ({ timestamp, visitor_id, session_id, event, path: routePath, referrer, duration_ms: Number(duration_ms) || 0 }));
    const performance = read(performancePath, ([timestamp, visitor_id, routePath, load_time_ms, lcp_ms, cls, inp_ms]) => ({ timestamp, visitor_id, path: routePath, load_time_ms: Number(load_time_ms) || 0, lcp_ms: Number(lcp_ms) || 0, cls: Number(cls) || 0, inp_ms: Number(inp_ms) || 0 }));
    const visitors = readVisitors();
    const contacts = read(contactsPath, ([timestamp, name, email, message]) => ({ timestamp, name, email, message })).reverse().slice(0, 5);
